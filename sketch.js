...
background(titS);
  ...
  if(gameState === "lv1"){
   ...
   if(player.isTouching(daggerGrp)){
    ...
    if(torch.x === player.x && birdGrp.isTouching(player)){
      birdGrp.visible = false;
      birdGrp.destroyEach();
    }
    ...
    if(keyCode === 32 && berry.isTouching(tunnel5)){
     ...
     temp_stars.x = player.x;
     temp_stars.y = player.y-5;
       console.log("hey")
    }
...
function createStars(){
  var star = createSprite(player.x,player.y-5,10,10);
  star.velocityX = 2;
  star.scale = 0.1;
  starGrp.add(star);
  return star;
